...
export default function CommitList({commits, setProjectPush, projectPush}) {
    ...
    const commitList = commits.map(commit => <CommitListItem key={commit._id} activeState={activeState} activeCommit={activeCommit} className="max-w-2xl" commit={commit} handleActiveState={handleActiveState} setProjectPush={setProjectPush} projectPush={projectPush} />)
return(
