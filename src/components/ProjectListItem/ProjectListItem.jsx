...
export default function ProjectListItem({project,reloadProject, setReloadProject }){
  const [projectOwner, setProjectOwner] = useState("") 
  const [projectMembers, setProjectMembers] = useState([])
  const [currentUser, setCurrentUser] = useState(getUser());
  const [editState, setEditState] = useState(false) // if edit button is clicked
 ...
 async function handleDelete(projectId) {
  ...
  },[editComplete, editProjectDescription])
  return (

    <div className="flex  justify-center w-10/12 ">
      {!editState ? 
            <Card className="mt-10 bg-gradient-to-r from-slate-900 to-slate-800   mb-10 w-full ">
      
              <div className="absolute top-2 -right-0 z-40">
                { project.user === currentUser._id ?
                    <Dropdown  >
                      <DropdownTrigger >
                        <Button className=" border-none h-10 hover:bg-none" variant="light">
              ...
              <CardHeader className="justify-between mt-6">
                <div className="flex   w-full">
                  {/* <Avatar isBordered radius="full" size="md" src="/avatars/avatar-1.png" />  */}  {/* Future login image */}
                  <div className="flex w-full  flex-col gap-1 items-start justify-center">
                    <h5 className=" ml-8 mb-2 text-xl tracking-tight text-default-400">
                      @{projectOwner.name}
                    </h5>
                    <h4 className=" bg-gradient-to-r from-stone-600 to-stone-800  flex items-center w-full h-12    ml-7 rounded-t-xl  text-2xl font-semibold leading-none text-default-600">
                      <span className="ml-4">
                        {thisProject.name}
                      </span>
                    </h4>
              ...
              <CardFooter className="gap-3 flex justify-between">
                ...
                <div className="flex ml-6">
                  {project.user === currentUser._id ? 
                      <Button>
                        <Link to={`${project._id}/addMembers`}>Add Members</Link>
                      </Button>
                    :
                      ""
                  }
          ...
          :
            <div className="flex flex-col items-center relative  w-2/4">
              <Button className="absolute left-0" onClick={()=> setEditState(!editState)}>
                X
              </Button>
