...
export default function Commit(){
    ...
    return(
        ...
        <div className="  flex justify-between items-center w-[67%] ml-4  ">
        ...
        <button onClick={()=> setCreateCommit(!createCommit)} >+</button>
        </div>
        {createCommit ? <div className="flex justify-center bg-opacity-60 rounded-xl absolute top-[30%]  w-[700px] z-40 h-[500px] bg-gray-600"> 
...
{/* <div className="flex justify-center bg-opacity-90 rounded-xl absolute top-[30%] right-[30%] w-[700px] z-40 h-[500px] bg-white ">
            <button className="absolute left-5 top-5" onClick={()=> setCreateCommit(!createCommit)}>X</button>    
            <form lassName="flex flex-col justify-center items-center" onSubmit={handleSubmit} >
            <h1 className="text-4xl -mt-10 mb-10">Create Commit</h1>
            <input className="w-[500px] mb-5 bg-gray-900 text-white" value={newCommit} onChange={(evt) => setNewCommit(evt.target.value)} />
            <button className="h-[60px]" type="submit">Submit</button>
        </form></div>  */}